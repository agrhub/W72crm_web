<template>
  <el-dialog
    ...
    <div class="label-input">
      <label class="label-title">Mô tả quyền</label>
      <el-input
        v-model="remark"
        :rows="2"
        :maxlength="300"
        type="textarea"
        placeholder="Vui lòng nhập mô tả quyền"/>
    </div>
    <label class="label-title">Thông tin quyền</label>
    ...
    <span
      ...
      <el-button @click="closeView">Hủy</el-button>
    </span>
  </el-dialog>
</template>
...
export default {
  ...
  methods: {
    ...
    sureClick() {
      if (!this.roleName) {
        this.$message.error('Vui lòng điền vào tên quyền')
      } else {
        this.loading = true
