<template>
  <flexbox v-loading="loading" :style="{height: contentHeight + 'px'}" class="calendar-box">
    <el-button type="warning" icon="el-icon-plus" @click="createEvents">Kế hoạch mới</el-button>
    <div class="box-left">
      <div class="left-title" >
        ...
        <xh-user-cell
          ...
          <flexbox slot="reference" class="user-box">
            <span class="username">{{ checkedUser[0]?checkedUser[0].realname + 'Lịch trình': 'Lịch trình của bạn' }}</span>
            <span :class="{ 'is-reverse' : $refs.xhuserCell && $refs.xhuserCell.showPopover }" class="el-icon-arrow-up icon"/>
          </flexbox>
        </xh-user-cell>
        <span v-else class="username">Lịch của bạn</span>
      </div>
      <div class="left-scroll">
        ...
        <div class="left-main">
          <div class="main-title" @click="showSys = !showSys">
            ...
            <span class="main-text">Nhãn mặc định</span>
        ...
        <div class="left-bottom">
          <div class="bottom-title" @click="showCus = !showCus">
            ...
            <span class="main-text">Nhãn tùy chỉnh</span>
      ...
      <div class="left-bottom-text">
        ...
        <span class="text-span">Custom types can be configured in the background</span>
      </div>
    </div>
    <div class="box-right">
      <FullCalendar
        ...
        all-day-text="Cả ngày"
        no-events-message="Không có sự kiện"
        locales="allLocales"
        locale="vi"
        class="calendar-main"
...
export default {
  ...
  data() {
    return {
      ...
      sysCheck: [
        ...
        { label: 'Khách hàng cần liên hệ' },
        { label: 'Hợp đồng hết hạn' },
        { label: 'Lên kế hoạch hoàn trả' }
      ],
      checkCusList: [],
      dayEventList: [],
      cusCheck: [],
      ...
      handler(val) {
        if (val.length === 1) {
          ...
          if (this.copyCheckCusList.length === 1) {
            ...
            this.$message.error('Vui lòng chọn ít nhất một loại')
          }
        } else {
          this.customFifter(val)
        }
  ...
  methods: {
    ...
    listDayFormat(data) {
      ...
      const week = moment(timestamp).format('dddd').replace('tuần', 'tuần')
    ...
    datesRender(info) {
      ...
      if (info.view.type === 'listDay') {
        if (info.el.textContent === 'Không có lịch') {
    ...
    handleData(list) {
      ...
      this.needData.leadsTimeList.forEach(date => {
        dataList.push({
          title: 'Các khách hàng tiềm năng cần liên hệ',
          startTime: date,
          eventId: -1,
          color: '#58DADA',
      ...
      this.needData.customerTimeList.forEach(date => {
        dataList.push({
          title: 'Các khách hàng cần liên hệ',
          startTime: date,
          eventId: -1,
          color: '#53D397',
      ...
      this.needData.businessTimeList.forEach(date => {
        dataList.push({
          title: 'Cơ hội kinh doanh cần liên hệ',
          startTime: date,
          eventId: -1,
          color: '#4586FF',
      ...
      this.needData.dealBusinessTimeList.forEach(date => {
        dataList.push({
          title: 'Cơ hội kinh doanh mong đợi',
          startTime: date,
          eventId: -1,
          color: '#8983F3',
