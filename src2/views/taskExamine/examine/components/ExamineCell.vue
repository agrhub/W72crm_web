          ...
          <div class="examine-relate__bd">
            <flexbox
              ...
              <div class="relate-cell__hd">
                ...
                <span>{{ `Có liên quan ${item.name}` }}：</span>
              </div>
...
export default {
  ...
  computed: {
    ...
    relateList() {
      ...
      const names = ['Danh bạ', 'Khách hàng', 'Cơ hội', 'Hợp đồng']
      const allList = []
    ...
    moreTypes() {
      ...
      if (this.data.permission) {
        ...
        if (this.data.permission.isUpdate) {
          mores.push({ type: 'edit', name: 'Chỉnh sửa', icon: 'edit' })
        }

        if (this.data.permission.isDelete) {
          mores.push({ type: 'delete', name: 'Xóa', icon: 'delete' })
        }
      }
      return mores
    },
