  ...
  <div :class="['mix-content', { 'unfold': !isUnfold }]">
    ...
    <div
      ...
      <el-select
        v-model="followType"
        clearable
        placeholder="Chọn phương pháp liên lạc">
        <el-option
      ...
      <el-date-picker
        ...
        :editable="false"
        type="datetime"
        placeholder="Chọn thời gian liên hệ tiếp theo"
    ...
    <div
      ...
      <el-button
        ...
        class="handle-button"
        icon="wk wk-picture"
        @click="barClick('img')">Hình ảnh</el-button>
      <el-button
        type="text"
        class="handle-button"
        icon="wk wk-file"
        @click="barClick('file')">Tập tin</el-button>
      <el-popover
        v-if="showBusiness"
        ...
        <el-button
          ...
          @click="barClick('business')">Liên kết cơ hội kinh doanh</el-button>
      </el-popover>
      <el-button
        v-debounce="sendClick"
        type="primary"
        class="send-btn">Lưu</el-button>
    </div>
...
export default {
  ...
  methods: {
    ...
    commonWordsSelect(data) {
      if (this.content) {
        this.$confirm('Những từ phổ biến bạn đã chọn sẽ ghi đè lên nội dung hiện tại?', 'Thông báo', {
      ...
      if (files.length) {
        for (let index = 0; index < files.length; index++) {
          ...
          if (
            ...
            this.$message.error('Vui lòng tải lên đúng loại tập tin')
            return
          }
        }
    ...
    deleteAllFile(type) {
      if (this.batchId) {
        this.$confirm('Xác nhận xóa tất cả?', 'Thông báo', {
          confirmButtonText: 'OK',
          cancelButtonText: 'Cancel',
          type: 'warning'
        })
