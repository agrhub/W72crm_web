          ...
          <template v-for="(item, index) in seciton.list">
            ...
            <div
              ...
              <div
                v-if="item.type == 2"
                class="activity-cell">
                <span class="activity-cell__label">{{ item.createTime }} {{ item.realname }} tạo {{ getActivityTypeName(item.activityType) }}：</span><span
                  class="activity-cell__content"
                  @click="checkCRMDetail(getActivityType(item.activityType), item.activityTypeId)">{{ item.activityTypeName || 'Xem chi tiết' }}</span>
              </div>
              <div
                v-else-if="item.type == 3"
                class="activity-cell">
                <span class="activity-cell__label">{{ item.createTime }} {{ item.realname }} cơ hội kinh doanh：</span>
                <span
                  class="activity-cell__content"
                  @click="checkCRMDetail('business', item.activityTypeId)">{{ item.activityTypeName }}</span>
                <span>{{ ` Giai đoạn chuyển thành ${item.content}` }}</span>
              </div>
              <log-cell
