<template>
  <el-dialog
          ...
          <el-row :key="index">
            ...
            <el-col :span="formItem.formType === 'datetime' || formItem.formType === 'date' || formItem.formType === 'map_address' ? 13 : 8">
              ...
              <el-input
                v-else
                v-model="formItem.value"
                placeholder="Vui lòng sử dụng nhiều điều kiện; tách biệt nhau"/>
            </el-col>
            <el-col
              :span="1"
              class="delete">
    ...
    <div
      v-if="saveScene"
      class="save">
      <el-checkbox v-model="saveChecked">Lưu bộ lọc</el-checkbox>
      <el-input
        v-show="saveChecked"
    ...
    handleConfirm() {
      ...
      for (let i = 0; i < this.form.length; i++) {
        ...
        if (!o.fieldName || o.fieldName === '') {
          this.$message.error('Tên trường đã lọc không được để trống！')
          return
        }
