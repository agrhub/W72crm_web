<template>
  <div v-loading="loading" class="b-cont">
    <div v-if="showSaveButton" class="b-cont__handle">
      <el-button @click.native="editCancel">Hủy</el-button>
      <el-button
          ...
          <template v-else>
            ...
            <div
              ...
              <div
                ...
                <flexbox
                  class="b-cell-b">
                  <div class="b-cell-name">Định vị</div>
                  <div
                    class="b-cell-value"
    ...
    getItemRulesArrayFromItem(item) {
      ...
      if (item.isNull == 1 && !this.ingnoreRequiredField(item)) {
        var validateIsNull = (rule, value, callback) => {
          ...
          if (isEmpty(value)) {
            callback(new Error(item.name + ' không được để trống'))
          } else {
            callback()
          }
        }
            ...
            filedValidatesAPI(validatesParams)
              .then(res => {
                ...
                if (res.data.status === 1) {
                  ...
                  callback(new Error(item.name + ' tồn tại'))
                }
              })
              .catch(error => {
                callback(new Error(error.msg ? error.msg : 'Lỗi xác minh'))
              })
          }
        }
        tempList.push({
      ...
      if (item.formType == 'number') {
        ...
        var validateCRMMoneyNumber = (rule, value, callback) => {
          ...
          if (!value || value == '' || regexIsCRMMoneyNumber(value)) {
            ...
            callback(new Error('Phần nguyên của đơn vị tiền tệ phải dưới 15 chữ số và phần thập phân phải ít hơn 2 chữ số'))
          }
        }
        tempList.push({
        ...
        var validateCRMMobile = (rule, value, callback) => {
          ...
          if (!value || value == '' || regexIsCRMMobile(value)) {
            ...
            callback(new Error('Định dạng số điện thoại sai'))
          }
        }
        tempList.push({
