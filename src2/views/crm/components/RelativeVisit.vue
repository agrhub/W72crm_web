  ...
  <div
    ...
    <flexbox
      ...
      <el-button
        ...
        icon="el-icon-plus"
        type="primary"
        @click="createClick">Lên lịch hẹn</el-button>
    </flexbox>
    <el-table
...
export default {
  ...
  methods: {
    ...
    getFieldList() {
      ...
      //   label: '回访编号'
      // })
      // this.fieldList.push({ prop: 'visitTime', width: '200', label: 'Thời gian gặp' })
      // this.fieldList.push({ prop: 'ownerUserName', width: '200', label: 'Nhân viên phụ trách' })
      // this.fieldList.push({ prop: 'returnVisitType', width: '200', label: '回访形式' })
      // this.fieldList.push({ prop: '客户满意度', width: '200', label: 'Mức độ hài lòng của KH' })
      // this.fieldList.push({ prop: 'contractNum', width: '200', label: 'Mã hợp đồng' })
      filedGetTableFieldAPI({
        label: 17
      }).then(res => {
        const list = res.data || []
