    ...
    <flexbox
      ...
      <div class="cell-bd">
        ...
        <p class="cell-bd__detail">
          ...
          <span
            v-else
            class="no-data">Không có số điện thoại</span>
        </p>
        <p class="cell-bd__detail">
          ...
          <span
            v-else
            class="no-data">Không có email</span>
        </p>
      </div>
      <span class="mark"><i class="wk wk-s-contacts" />Danh bạ chính</span>
    </flexbox>
...
export default {
  ...
  computed: {
    ...
    // 空数据按钮文字 为空不展示
    emptyBtnName() {
      return this.$listeners.add && !this.isSeas ? 'Danh bạ mới' : ''
    },

    emptyName() {
      return (
        {
          customer: 'Không có dữ liệu khách hàng',
          business: 'Không có dữ liệu cơ hội kinh doanh'
        }[this.crmType] || 'Không có dữ liệu'
      )
    }
  },
  watch: {
