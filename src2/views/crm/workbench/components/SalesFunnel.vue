  ...
  <div
    ...
    <flexbox class="card-title">
      ...
      <div class="card-title-center text-one-ellipsis">
        ...
        <template v-if="businessId">
          (Đơn vị kinh doanh：{{ businessName }})
        </template>
      </div>
    ...
    <flexbox justify="center" class="info-box">
      <flexbox direction="column" class="info-item">
        ...
        <div class="value">{{ funnelData.winSingle }} đồng</div>
      </flexbox>
      <flexbox direction="column" class="info-item">
        <div class="label">Thất bại</div>
        <div class="value">{{ funnelData.loseSingle }} đồng</div>
      </flexbox>
    </flexbox>
  </div>
</template>
  ...
  data() {
    return {
      chartOption: {
        tooltip: {
          trigger: 'item',
          formatter: '{b} <br/> Số lượng cơ hội: {c} lượt'
        },
        calculable: true,
        legend: {
          data: [],
    ...
    getData() {
      ...
      crmIndexFunnelAPI({
        ...
        for (let index = 0; index < this.dataList.length; index++) {
          ...
          data.push({
            name: (element.name || '') + ' (' + element.money + ' đồng)',
            value: element.count
          })
