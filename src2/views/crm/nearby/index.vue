    ...
    <div v-loading="loading" class="map-content">
      <flexbox align="stretch" class="map-content--title">
        <flexbox-item>
          ...
          <el-button type="text" class="el-button--text" @click="changeAddressShow = true">Chọn lại địa chỉ</el-button>
          <span class="place-text">Gần đây&nbsp;</span>
          <el-popover
            ...
            <div class="type-popper">
              ...
              <div
                ...
                <el-input
                  ...
                  <template slot="append">Mét</template>
                </el-input>
                <el-button type="primary" @click="radiusChange(null)">Lưu</el-button>
              </div>
            </div>
            <el-input
              slot="reference"
          ...
          <span class="place-text">&nbsp;Khách hàng</span>
        </flexbox-item>

        <div class="title--right">
          <label class="title-label">Loại khách hàng： </label>
            ...
            <ul ref="nearbyList" :style="{height: mapHeight + 'px'}">
              <li
                ...
                <div class="map-box--content">
                  ...
                  <div class="map-info--content">Người phụ trách: {{ item.ownerUserName }}</div>
                  <div :title="item.detailAddress" class="map-info--content">{{ item.detailAddress }}</div>
                  <div class="map-info--content">Bán kính khu vực: {{ item.distance / 1000 }} km</div>
                </div>
              </li>
            </ul>
          </div>
...
export default {
  ...
  mounted() {
    getGoogleMap()
      .then(() => {
        var map = new GMap.Map('choicemap')
        var point = new GMap.Point(116.404, 39.915)
  ...
  methods: {
    ...
    */
    setCircle() {
      ...
      const Circle = new GMap.Circle(this.centerPoint, this.mapData.radius, {
        strokeColor: '#2362FB',
        fillColor: '#2362FB',
        strokeWeight: 2,
        fillOpacity: 0.05,
    ...
    addMarkerLabel() {
      ...
      ComplexCustomOverlay.prototype = new GMap.Overlay()
      // 地图发生变化的时候会触发方法
      ComplexCustomOverlay.prototype.initialize = function(map) {
        ...
        div.style.zIndex = this._zIndex = GMap.Overlay.getZIndex(this._point.lat)
      ...
      this.mapList.forEach(item => {
        const itemPoint = new GMap.Point(item.lng, item.lat)
