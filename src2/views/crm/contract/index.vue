<template>
  <div>
    <c-r-m-list-head
      ...
      placeholder="Vui lòng nhập tên khách hàng / số hợp đồng / tên hợp đồng"
      main-title="Hợp đồng mới"
...
export default {
  ...
  computed: {
    moneyDes() {
      ...
      let moneyDes = ''
      if (canCheckMoney) {
        moneyDes = `Tổng số tiền của hợp đồng：${separator(contractMoney || 0)}`
      }

      if (canCheckReceivedMoney) {
        moneyDes += moneyDes ? ` / ` : ''
        moneyDes += `Số tiền đã thanh toán：${separator(receivedMoney || 0)}`
      }

      if (canCheckUnReceivedMoney) {
        moneyDes += moneyDes ? ` / ` : ''
        moneyDes += `Số tiền chưa thanh toán：${separator(unReceivedMoney || 0)}`
      }
