      ...
      <div class="sm-main">
        <div class="sm-main__hd">
          ...
          <el-button
            ...
            class="notice-btn"
            type="text"
            @click="createNotice">Thông báo mới</el-button>
        </div>
        ...
        <flexbox class="sm-main__ft">
          <div class="switch-read">
            <el-switch
              v-model="isUnRead"
              @change="refreshList"/>
            <span class="switch-read__title">Chỉ hiển thị các tin nhắn chưa đọc</span>
          </div>
          <el-dropdown
            trigger="click"
            ...
            <el-dropdown-menu slot="dropdown">
              <el-dropdown-item
                icon="el-icon-check"
                command="read">{{ `Tất cả ${currentMenu.label == 'all' ? '' : currentMenu.name} đánh dấu là đã đọc` }}</el-dropdown-item>
              <el-dropdown-item
                icon="wk wk-s-delete"
                command="delete">{{ `Xóa bỏ ${currentMenu.name} tin nhắn` }}</el-dropdown-item>
            </el-dropdown-menu>
          </el-dropdown>
        </flexbox>
      </div>
...
export default {
  ...
  computed: {
    ...
    title() {
      if (this.onlyAnnouncement) {
        return this.unreadNums.announceCount && this.unreadNums.announceCount > 0 ? `Thông báo （${this.unreadNums.announceCount}）` : 'Thông báo'
      }
      return this.unreadNums.allCunt > 0 ? `Tin nhắn （${this.unreadNums.allCunt}）` : 'Tin nhắn'
    },
  ...
  methods: {
    ...
    deleteMessageClick(messageId, index) {
      this.$confirm('Xác nhận xóa tin nhắn?', 'Thông báo', {
    ...
    allDeleteClick() {
      if (this.currentMenu) {
        ...
        this.$confirm(`Xác nhận xóa tất cả ${name}?`, 'Thông báo', {
          confirmButtonText: 'OK',
          cancelButtonText: 'Cancel',
          type: 'warning'
        })
