...
export default {
  ...
  filters: {
    detail: function(data) {
      if (data.stepType == 2) {
        return data.userList.length + 'Người ký duyệt chỉ định'
      } else if (data.stepType == 3) {
        return data.userList.length + 'Mội người cùng ký duyệt'
      } else if (data.stepType == 1) {
        return 'Người quản lý ký duyệt'
      } else if (data.stepType == 4) {
        return 'Cấp trên của người quản lý ký duyệt'
      }
    },
    step: function(index) {
      return 'Quay về' + Nzhcn.encodeS(index) + 'Tiếp theo'
    },
