...
export default {
  ...
  filters: {
    detailName: function(data) {
      if (data.stepType == 2) {
        return data.userList.length + 'Người duyệt chỉ định'
      } else if (data.stepType == 3) {
        return data.userList.length + 'Mọi người cùng ký duyệt'
      } else if (data.stepType == 1) {
        return 'Quản lý trực tiếp ký duyệt'
      } else if (data.stepType == 4) {
        return 'Cấp trên của quản lý ký duyệt'
