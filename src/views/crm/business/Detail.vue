    ...
    <div
      ...
      <flexbox
        ...
        <c-r-m-detail-head
          ...
          @close="hideView">
          <template slot="name">
            <el-tooltip :content="detailData.star == 0 ? 'Thêm đánh dấu': 'Bỏ đánh dấu'" effect="dark" placement="top">
              <i
...
export default {
  ...
  data() {
    return {
      ...
      headDetails: [
        ...
        { title: 'Số tiền cơ hội （đông）', value: '' },
        { title: 'Đơn vị kinh doanh', value: '' },
  ...
  methods: {
    ...
    handleStatuChange(item, index) {
      ...
      if (!item.isdoing) {
        var message = 'Xác nhận đưa ' + item.name + ' vào kịch bản?'
    ...
    handleStatuResult(item, index) {
      ...
      if (item.type == 2 || item.type == 3) {
        var message = 'Vui lòng điền ' + item.name + ' lý do：'
        var title = item.name + ' lý do'
        ...
        this.$confirm('Xác nhận đưa cơ hội kinh doanh hiện tại ' + item.name + '?', 'Thành công', {
          confirmButtonText: 'OK',
          cancelButtonText: 'Cancel',
          type: 'warning'
        })
