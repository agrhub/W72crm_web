<template>
  <div class="table-head-container">
    <flexbox
      ...
      class="th-container">
      <slot name="custom"/>
      <div v-if="showSceneView">Bộ lọc：</div>
      <el-popover
        v-if="showSceneView"
        v-model="showScene"
        trigger="click"
      ...
      <el-button
        ...
        {{ `${sortData.column.label}${{ascending: 'Tăng dần', descending: 'Giảm dần'}[sortData.order]}` }}<i style="margin-left: 5px;" class="el-icon-close"/>
      </el-button>
      <filter-form
        v-if="showFilterView"
  ...
  methods: {
    ...
    selectionBarClick(type) {
      if (type == 'transfer') {
        ...
        if (type == 'transform') {
          ...
          message = this.isSeas ? 'Nếu có các danh bạ của khách hàng, các danh bạ sẽ bị xóa. Xác nhận xóa？' : `Xóa phần đã chọn ${this.selectionList.length} mục？`
        } else if (type == 'lock') {
          message = 'Bạn có chắc chắn muốn khóa những khách hàng này không? Sau khi bị khóa, nó sẽ không đưa vào hồ chứa.'
        } else if (type == 'unlock') {
        ...
        // 团队操作
        this.teamsTitle = 'Xóa thành viên trong nhóm'
        this.teamsDialogShow = true
      } else if (type == 'alloc') {
        // 公海分配操作
    ...
    getSelectionHandleItemsInfo() {
      const handleInfos = {
        ...
        start: {
          name: 'Đặt lên kệ hàng',
          type: 'start',
          icon: 'shelves'
        },
        disable: {
          name: 'Gở khỏi kệ hàng',
          type: 'disable',
          icon: 'sold-out'
        },
        deal_status: {
    ...
    getDefaultSceneName() {
      if (this.crmType == 'leads') {
        ...
        return 'Tất cả danh bạ'
      } else if (this.crmType == 'business') {
        return 'Tất cả cơ hội kinh doanh'
      } else if (this.crmType == 'contract') {
