  ...
  <el-container
    ...
    <el-header >
      <template v-if="isPreview">
        <el-button type="primary" @click="handleClick('download')">{{ `Tải xuống ${perviewTypeName}` }}</el-button>
        <el-button type="primary" @click="handleClick('print')">In</el-button>
        <el-button @click="handleClick('goback')">Hủy bỏ</el-button>
      </template>
      <template v-else>
        <span class="select-label">Chọn một mẫu</span>
        <el-select
          v-model="templateId"
          style="width: 170px;"
        ...
        <el-button class="preview-button" type="primary" @click="printPreviewClick">Xem trước khi in</el-button>
      </template>
    </el-header>
    <el-main>
      <div class="main-container">
        ...
        <div v-show="!isPreview" class="main-tips">Bạn có thể điều chỉnh văn bản mẫu trên trang hiện tại và in nó sau khi xác nhận rằng mẫu là chính xác</div>
      ...
      if (type == 'goback') {
        ...
        if (this.srcData) {
          ...
          crmFileDownByPathAPI(data).then(res => {
            ...
            downloadFileWithBuffer(blob, `Tài liệu.${ext}`)
          }).catch(() => {})
        }
      }
    }
