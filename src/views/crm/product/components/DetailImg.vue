  ...
  <div v-loading="loading" class="b-cont">
    <div class="detail-box">
      <div class="detail-item">
        <label class="detail-label"> Hình ảnh sản phẩm</label>
        ...
        <div class="detail">
          ...
          <div class="detail-tip">Ảnh tải lên để xuất：290px（rộng） x 220px(cao) （Chỉ có thể tải lên nhiều nhất <span>{{ 9 - primaryList.length }}</span> ảnh）</div>
        </div>
      </div>
    </div>
    <div class="detail-box">
      <div class="detail-item">
        <label class="detail-label"> Hình ảnh chi tiết sản phẩm</label>
        <div class="detail">
          <flexbox class="detail-images">
          ...
          <div class="detail-tip">Hình ảnh tải lên để xuất：750px(rộng) x 600px(cao)（Chỉ có thể tải lên tối đa <span>{{ 9 - primaryList.length }}</span> ảnh）</div>
        </div>
      </div>
    </div>
  </div>
  ...
  methods: {
    ...
    upLoad(event) {
      ...
      if (this.type === 'main' && files.length + this.primaryList.length > 9) {
        ...
        this.$message.error('Chỉ tải lên tối đa 9 hình ảnh')
    ...
    deleteImg(type, index) {
      ...
      this.$confirm('Thao tác này sẽ xóa ảnh vĩnh viễn, bạn có muốn tiếp tục không?', 'Thông báo', {
