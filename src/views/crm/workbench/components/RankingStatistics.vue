<template>
  <div
    ...
    <flexbox class="card-title">
      ...
      <div class="card-title-center text-one-ellipsis">Bảng xếp hạng</div>
    ...
    <div class="card-desc">
      {{ `Thống kê xếp hạng ${optionName}` }}
    </div>
    <div class="content">
      <div class="title">{{ `Bảng xếp hạng ${optionName}` }}</div>
      <el-table
        :data="data"
        height="233"
        style="width: 100%">
        ...
        <el-table-column
          v-if="rateShow"
          label="Tỷ lệ hoàn thành mục tiêu（%）"
          prop="rate" />
      </el-table>
      <div
        class="my-ranking">
  ...
  data() {
    return {
      ...
      options: [
        ...
        { name: 'Số lượng hợp đồng', value: 3 },
        { name: 'Số lượng khách hàng mới', value: 4 },
        { name: 'Thêm danh bạ mới', value: 5 },
        // { name: 'Thêm cơ hội mới', value: 6 },
        // { name: '新增商机金额', value: 7 },
        { name: 'Số lượng theo dõi mới', value: 8 }
      ],
      optionName: 'Số tiền thanh toán',
      optionValue: 2
    }
  },
  computed: {
