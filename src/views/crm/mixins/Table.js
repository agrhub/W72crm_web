...
export default {
  ...
  methods: {
    ...
    exportInfos() {
      ...
      const loading = Loading.service({ fullscreen: true, text: 'Xuất file...' })
      request(params)
        .then(res => {
    ...
    toggleStar(data) {
      ...
      request(params).then(() => {
        this.loading = false
        this.$message.success(data.star > 0 ? 'Bỏ theo dõi thành công' : 'Theo dõi thành công')
