  ...
  <flexbox justify="flex-start" class="filtrate-content">
    ...
    <template v-if="showFilterView">
      ...
      <el-select
        ...
        v-model="dataSelect">
        <el-option
          v-for="item in [{label:'Theo phòng ban', value:1},{label:'Theo nhân viên', value:2}]"
          :key="item.value"
          :label="item.label"
          :value="item.value"/>
      </el-select>
      <xh-structure-cell
        v-if="dataSelect == 1"
        radio
        placeholder="Chọn phòng ban (mặc định là phòng ban này và các phòng ban cấp dưới）"
      ...
      <xh-user-cell
        ...
        radio
        class="xh-user-cell"
        placeholder="Chọn nhân viên (mặc định là nhân viên và cấp dưới)
"
