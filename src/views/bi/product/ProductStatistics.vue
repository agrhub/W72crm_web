...
export default {
  ...
  data() {
    return {
      ...
      headFieldList: [
        { field: 'categoryName', name: 'Danh mục', width: '115px' },
        { field: 'productName', name: 'Tên sản phẩm', width: '115px' },
        { field: 'contractNum', name: 'Số lượng hợp đồng', width: '115px' },
        { field: 'num', name: 'Số lượng', width: '115px' },
        { field: 'total', name: 'Thành tiền', width: '115px' }
      ],
      list: [],
      //
      spanList: [],
    ...
    handleShowInfo() {
      ...
      for (let index = 0; index < this.list.length; index++) {
        ...
        if (spanList.length == 0) {
          ...
          subMoney = parseFloat(element.total)
          allCount = parseFloat(element.num) // 系列
          allMoney = parseFloat(element.total)

          spanList.push({ rowspan: 1, productRowspan: 1 })
          newList.push(element) // 真实数据
        } else if (element.categoryName != this.list[index - 1].categoryName) {
          // 系列改变时候的逻辑
          /** 上一个最后产品的处理 */
          var preItem = spanList[seriesIndex]
          preItem.rowspan += 1
          newList.push({ productPrice: 'Giá bán', productNum: subCount, productSubtotal: subMoney }) // 产品小计数据
          spanList.push({ rowspan: 0, productRowspan: 1, isSum: true }) // 产品小计style

          newList.push({ productPrice: 'Giá bán', productNum: allCount, productSubtotal: allMoney }) // 系列小计数据
          ...
          if (element.productId == this.list[index - 1].productId) {
            ...
            subMoney = floatAdd(subMoney, parseFloat(element.total))
            allCount = floatAdd(allCount, parseFloat(element.num)) // 系列
            allMoney = floatAdd(allMoney, parseFloat(element.total))
            ...
            subMoney = parseFloat(element.total) // 开始了一个新的产品  所以没有 清空数据
      ...
      newList[newList.length - 1].productPrice = 'Giá bán'
