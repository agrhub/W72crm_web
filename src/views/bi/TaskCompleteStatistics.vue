  ...
  <div
    ...
    <flexbox class="handle-bar">
      <flexbox justify="flex-start" class="title-box">
        ...
        <span class="text">Đạt được các mục tiêu hiệu suất</span>
      </flexbox>
      <el-date-picker
        v-model="dateSelect"
        :clearable="false"
      ...
      <el-select
        v-model="dataSelect">
        <el-option
          v-for="item in [{label:'Phòng ban', value:1},{label:'Nhân viên', value:2}]"
          :key="item.value"
          :label="item.label"
          :value="item.value"/>
      </el-select>
...
export default {
  ...
  data() {
    return {
      ...
      list: [],
      fieldList: [
        // { field: 'month', name: 'Tháng' },
        // { field: 'receivables', name: 'Số tiền hợp đồng (đồng)' },
        // { field: 'achievement', name: 'Số tiền mục tiêu (đồng)' },
        // { field: 'rate', name: 'Tỷ lệ hoàn thành (%)' }
      ],
  ...
  mounted() {
    ...
    const keysName = ['Tên', 'Mục tiêu hàng năm', 'Quý I', 'Tháng 1', 'Tháng 2', 'Tháng 3', 'Quý II', 'Tháng 4', 'Tháng 5', 'Tháng 6', 'Quý III', 'Tháng 7', 'Tháng 8', 'Tháng 9', 'Quý IV', 'Tháng 10', 'Tháng 11', 'Tháng 12']
    ...
    for (let index = 0; index < keysName.length; index++) {
      ...
      if (index == 0) {
        ...
        const children = [
          { field: `achievement${keys[index]}`, name: 'Mục tiêu' },
          { field: `money${keys[index]}`, name: 'Hoàn thành' },
          { field: `rate${keys[index]}`, name: 'Tỷ lệ hoàn thành' }
        ]
  ...
  methods: {
    ...
    refreshTableHeadAndChartInfo() {
      this.fieldList[1].name =
        this.typeSelect == 1 ? 'Giá trị hợp đồng (đồng)' : 'Số tiền thanh toán (đồng)'
      this.axisOption.legend.data[0] =
        this.typeSelect == 1 ? 'Giá trị hợp đồng' : 'Số tiền thanh toán'
    ...
    initAxis() {
      ...
      var option = {
        ...
        legend: {
          data: ['Số tiền hợp đồng', 'Mục tiêu', 'Tỷ lệ hoàn thành'],
          bottom: '0px',
          itemWidth: 14
        },
        grid: {
