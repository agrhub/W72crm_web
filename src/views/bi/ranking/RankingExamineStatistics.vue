<template>
  <div
    ...
    <filtrate-handle-view
      ...
      title="Xếp hạng các chuyến công tác"
      class="filtrate-bar"
      module-type="ranking"
      @load="loading=true"
      @change="getDataList"/>
    <div class="content">
      <div class="content-title">Xếp hạng số lượng chuyến công tác (thống kê theo nhân viên và thời gian đi công tác)</div>
      <div
...
export default {
  ...
  mounted() {
    this.fieldList = [
      ...
      { field: 'count', name: 'Số chuyến công tác （chuyến）' }
    ]
    this.initAxis()
  },
  methods: {
    ...
    initAxis() {
      ...
      this.axisOption.tooltip.formatter = '{b} : {c} chuyến'
      this.axisOption.xAxis[0].name = '（chuyến）'
      this.chartObj.setOption(this.axisOption, true)
    },
