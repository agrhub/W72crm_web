          ...
          <template v-if="data && data.formType == 'subTask'">
                ...
                <flexbox
                  ...
                  <div class="edit-del-box">
                    <span
                      ...
                      @click="subTaskDelete(item,index, data)">Xóa</span>
                  </div>

                  <div
                    v-if="item.stopTime"
                    style="margin-left: 8px;"
                    class="bg-color task-bg-color">{{ item.stopTime | moment("YYYY-MM-DD") }} Ngày kết thúc</div>
                  <xr-avatar
                    v-if="item.mainUser"
            ...
            <div v-if="addSubtasks">
              <span
                ...
                <i class="wk wk-l-plus" />
                <span class="label">Thêm công việc con</span>
              </span>
            </div>
            <sub-task
              v-else
...
export default {
  ...
  data() {
    return {
      ...
      fieldForm: {},
      fieldRules: {
        name: { required: true, message: 'Tên nhiệm vụ không được để trống', trigger: ['blur', 'change'] }
      },
      addSubtasks: true,
      batchId: guid(), // 批次ID
      allData: {},
  ...
  computed: {
    ...
    title() {
      return this.action.type === 'update' ? 'Chỉnh sửa công việc' : 'Công việc mới'
    }
  },
