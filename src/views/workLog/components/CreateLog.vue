  ...
  <div
    ...
    <span
      ...
      <i
        ...
        style="color: #2362FB " />
      <span class="complete-btn__name">{{ activeMap[activeTab] }}Hoàn thành<span class="value">{{ completeInfo.completeNum }}</span>/<span class="value">{{ completeInfo.totalNum }}</span></span>
    </span>
    ...
    <template v-if="!showMore">
      <div class="content-box">
        <flexbox
          ...
          <span class="info">Vui lòng nhập nội dung báo cáo</span>
        </flexbox>
      </div>
    </template>
    <template v-else>
      ...
      <div class="add-control">
        ...
        <div class="control-item">
          <span class="cursor-default">Gửi đến：</span>
          <span
          ...
          <members-dep
            ...
            <span slot="membersDep">
              <el-tooltip
                ...
                popper-class="tooltip-change-border">
                <div slot="content">
                  <span>{{ `Gửi đến ${sendUserList.length} người` }}</span>
                </div>
                <i
        ...
        <span class="control-relevance">
          <span>Tóm tắt bán hàng liên quan</span>
          <el-switch
...
export default {
  ...
  methods: {
    ...
    handleTabToggle() {
      const map = {
        day: ['Hôm nay', 'Ngày mai'],
        week: ['Tuần này', 'Tuần sau'],
        month: ['Tháng này', 'Tháng sau']
      }
      const arr = map[this.activeTab]
      this.textFormKeyList = [
        { label: `Nội dung công việc ${arr[0]}：`, key: 'content' },
        { label: `Nội dung công việc ${arr[1]}：`, key: 'tomorrow' },
        { label: 'Những vấn đề bắt gặp：', key: 'question' }
      ]
    },
    ...
    handleCreate() {
      if (!this.form.content && !this.form.tomorrow && !this.form.question) {
        this.$message.error('Vui lòng điền vào nội dung báo cáo')
        return
      }
      ...
      if (files.length) {
        for (let index = 0; index < files.length; index++) {
          ...
          if (
            ...
            this.$message.error('Vui lòng tải lên đúng loại tệp')
            return
          }
        }
    ...
    deleteAllFile(type) {
      if (this.batchId) {
        this.$confirm('Xác nhận xóa toàn bộ?', 'Thông báo', {
          confirmButtonText: 'OK',
          cancelButtonText: 'Cancel',
          type: 'warning'
        })
