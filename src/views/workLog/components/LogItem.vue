  ...
  <div class="log-item">
    <div class="main">
      <div class="user">
        ...
        <div class="box">
          ...
          <div class="time">
            Tạo báo cáo lúc {{ data.createTime }} <!--{{ data.isRead === 1 ? 'Đã đọc' : 'Chưa đọc' }}-->
          </div>
          <div
            class="handle">
            <el-button
              v-if="showHistoryBtn"
              type="text"
              @click="checkHistoryClick">Xem báo cáo trước đây</el-button>
          </div>
          <div class="comment-status">
            <span class="icon wk wk-task" />
            <span>{{ getCategory(data.categoryId) }}-{{ data.replyNum === 0 ? 'Chưa được đánh giá': 'Đã xem xét' }}</span>
    ...
    <div class="footer">
      ...
      <el-button
        ...
        icon="wk wk-message"
        class="replay-btn"
        @click="replayClick">{{ 'Bình luận' + (replyTotal > 0 ? `(${replyTotal})` : '') }}</el-button>
    </div>
    <div
      v-if="showReply"
      class="reply-wrapper">
  ...
  computed: {
    ...
    logTitleName() {
      return {
        ...
        2: 'Tuần này',
        3: 'Tháng này'
      }[this.data.categoryId] + 'Nội dung công việc'
    },
    logNextTitleName() {
      return {
        1: 'Ngày mai',
