...
export default {
  ...
  data() {
    return {
      ...
      openOptions: [
        {
          value: 0,
          label: 'Riêng tư： Chỉ các thành viên đã tham gia mới có thể xem mục này'
        },
        {
          value: 1,
  ...
  computed: {
    ...mapGetters(['userInfo']),
    title() {
      return this.id ? 'Chỉnh sửa dự án' : 'Tạo dự án'
    }
  },
  ...
  methods: {
    ...
    submitBtn() {
      if (!this.name) {
        this.$message.error('Vui lòng nhập tên dự án')
        return
      }
      this.loading = true
      const params = {
    ...
    corverDelete() {
      this.$confirm('Bạn có chắc chắn muốn xóa ảnh đại diện này không?', 'Thông báo', {
