    ...
    <el-dialog
      ...
      <div v-if="!showRepeat">
        <el-form ref="ruleForm" :model="form" :rules="rules" label-position="left">
          <el-form-item prop="title" label="Lên lịch nội dung sự kiện">
            <el-input
              v-model="form.title"
              maxlength="100"
              placeholder="Vui lòng nhập nội dung lịch trình"
              style="width: 330px"
              show-word-limit/>
          </el-form-item>
          <el-form-item label="Loại lịch biểu" prop="typeId">
            <div class="color_change">
              <span :style="{backgroundColor: colorItem}" class="custom_left"/>
            </div>
            <el-select
              v-model="form.typeId"
              placeholder="Chọn loại lịch biểu"
              class="select_color">
              <el-option
          ...
          <el-form-item label="Những người tham gia" prop="ownerUserIds">
            <xh-user-cell
              :value="checkedUser"
              :radio="false"
          ...
          <el-form-item v-if="!choseMore" style="margin-top: -10px">
            <el-button
              ...
              type="text"
              @click="choseMore = true">Thêm các tùy chọn khác</el-button>
          </el-form-item>
          <template v-if="choseMore">
            <!-- <el-form-item v-if="editAll" label="Lặp lại">
              <el-select
            ...
            <template v-for="(noticeItem,index) in notice">
              <el-form-item :key="index" label="Nhắc trước">
                <el-select v-model="notice[index].value" placeholder="Lựa chọn thời gian" class="time_select">
                  <el-option
                ...
                <el-select
                  ...
                  <el-option :value="1" label="phút"/>
                  <el-option :value="2" label="giờ"/>
                  <el-option :value="3" label="ngày"/>
                </el-select>
                <el-button
                  v-if="index != 0"
                  icon="el-icon-delete"
            ...
            <div class="new_notice" @click="addNewNotice">+ Thêm lời nhắc mới</div>
            <el-form-item label="Thông tin liên quan">
              <related-business
                :is-task="true"
                :all-data="allData"
                show-foot
          ...
          <el-form-item v-if="choseMore" style="margin-top: -10px">
            <el-button style="font-size: 14px" type="text" @click="choseMore = false">Ẩn các tùy chọn khác</el-button>
          </el-form-item>
        </el-form>
      </div>
      <repeat
      ...
      <div slot="footer" class="dialog-footer">
        <el-button :disabled="disabled" type="primary" @click="sure">Tạo</el-button>
        <el-button @click="cancle">Hủy</el-button>
      </div>
    </el-dialog>
  </div>
...
export default {
  ...
  data() {
    return {
      ...
      rules: {
        title: [
          { required: true, message: 'Nội dung lịch biểu không được để trống', trigger: 'blur' }
        ],
        typeId: [
          { required: true, message: 'Vui lòng chọn loại lịch biểu', trigger: 'change' }
        ],
        startTime: [
          { required: true, message: 'Vui lòng chọn thời gian bắt đầu', trigger: 'blur' }
        ],
        endTime: [
          { required: true, message: 'Vui lòng chọn thời gian kết thúc', trigger: 'blur' }
        ],
        ownerUserIds: [
          { required: true, message: 'Vui lòng chọn người tham gia', trigger: 'change' }
        ]
      },
      disabled: false,
      pickerOptions: {
  ...
  methods: {
    ...
    unbindRelatedInfo(field, item, index) {
      this.$confirm('Hủy thông tin liên kết?', 'Thông báo', {
    ...
    createSchedule() {
      ...
      request(params).then(res => {
        this.$message.success('Tạo hoặc chỉnh sửa lịch biểu thành công')
        this.disabled = false
