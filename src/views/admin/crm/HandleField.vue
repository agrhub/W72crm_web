<template>
  <flexbox
    ...
    orient="vertical"
    align="stretch">
    <div class="title">{{ 'Chỉnh sửa '+getTitle() }}</div>
    <el-container class="wrapper">
      <el-aside class="left">
        <div class="mini-title">Các trường thông tin</div>
        <ul>
          <draggable
            :list="fieldList"
      ...
      <el-container
        ...
        <el-header>
          ...
          <el-button
            ...
            type="primary"
            style="padding: 8px 22px;border-radius:2px;">Lưu</el-button>
          <el-button
            style="padding: 8px 22px;border-radius:2px;"
        ...
        <el-main>
          ...
          <p
            ...
            class="no-list">Nhấp hoặc kéo từ bên trái để thêm trường</p>
        </el-main>
      </el-container>
      <el-aside
        class="right"
        width="310px">
        <div class="mini-title">Thuộc tính trường</div>
        <field-info
          v-if="form"
          :field="form"
      ...
      for (let index = 0; index < tempFieldArr.length; index++) {
        ...
        if (item.name !== '' && item.name !== null && item.name !== undefined) {
          ...
          if (limitFields.includes(item.name)) {
            this.$message({
              message: `[${item.name}] Tên định danh của trường tùy chỉnh giống với trường hệ thống, vui lòng sử dụng các trường khác！`,
              type: 'error'
            })
            return
          } else if (names.includes(item.name)) {
            this.$message({
              message: `Dòng ${(index + 1)} cột [${item.name}] Tên mã nhận dạng trường tùy chỉnh trùng lặp`,
              type: 'error'
            })
            return
          } else {
        ...
        if (!item.name) {
          this.$message({
            type: 'error',
            message: `Dòng ${index + 1} trường tùy chỉnh, tên thẻ không được để trống`
          })
          return
          ...
          if (!regexIsCRMMobile(item.defaultValue)) {
            this.$message({
              message: `Dòng ${(index + 1)} cột （${item.name}） định dạng điện thoại mặc định được nhập vào trường tùy chỉnh không chính xác`,
              type: 'error'
            })
            return
          }
          ...
          if (!regexIsCRMEmail(item.defaultValue)) {
            this.$message({
              message: `Dòng ${(index + 1)} cột （${item.name}） định dạng hộp thư mặc định được nhập vào trường tùy chỉnh không chính xác`,
              type: 'error'
            })
            return
          }
